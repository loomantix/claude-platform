...
Run an autonomous Claude relay over the `/issues` ready queue. Each iteration claims an issue, spawns a fresh Claude session in `--permission-mode bypassPermissions`, lets it work, then pushes the result to a shared collection branch. After the loop, opens a `Ralph: <branch>` PR with the closed-issues + commit-log summary.
...
Defaults: 10 iterations, auto-generated collection branch (`ralph-<timestamp>-<rand>`), ready-queue-only.
...
1. **`RALPH.md` at the repo root** — repo-specific agent instructions (codebase conventions, build commands, test invocation, deployment quirks). The Claude prompt is fixed: `Read @RALPH.md and follow the instructions. Your assigned issue is #N. Run 'gh issue view N' to see the full description, then complete it.` If `RALPH.md` is missing, the script exits before claiming work.
...
1. Sync the worktree with `origin/<collection-branch>` — fetches and fast-forwards. If the remote was force-pushed, cherry-picks our local commits onto the new tip (with a pre-reset SHA snapshot so a failed cherry-pick restores the original chain rather than leaving partial replay). Genuine merge conflicts fail loud; the eventual push surfaces persistent ones via the `PUSH_FAILURES` counter.
...
4. Spawn `claude --chrome --permission-mode bypassPermissions --print "Read @RALPH.md..." --output-format stream-json` and stream the events through `jq` for colored display. The Claude PID is tracked so `Ctrl-C` interrupts the loop cleanly.
...
If any commits accumulated on `origin/<collection-branch>` past `origin/<default-branch>`, opens a `Ralph: <collection-branch>` PR (or attaches to an existing one) with:
