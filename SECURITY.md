...
1. **`.claude/lint-skill-content.py`** — CI lint (`skill-content` job) that flags **new lines** in skill/agent files for fetch-and-execute, reverse shells, credential reads (filesystem paths — `~`, `~user`, `$HOME`, `/home/<user>`, `/root`, `/Users/<user>` × `.aws|.ssh|.gnupg|.netrc|.kube|.docker|.npmrc|.config/{gh,gcloud,kubectl,kube,docker,npm}`, including bash brace-expansion forms — and credential env-var names like `AWS_SECRET_ACCESS_KEY`), shell dereferences of credential env vars (`$GITHUB_TOKEN`, `${NPM_TOKEN}`, etc.), environment exfiltration, base64-decode-exec, off-allowlist URLs (case-insensitive scheme; hostname from `urllib.parse.urlsplit` so the real host of `https://github.com@attacker.io` is identified as `attacker.io`, not `github.com`), defanged URLs, and raw `curl`/`wget`/`nc`/`socat`/`telnet`. Runs on every PR. Only added lines are scanned, so legacy patterns can't retroactively break the gate but new ones must be clean. The patterns are documented in [`CONTRIBUTING.md`](./CONTRIBUTING.md#skill-content-policy).
