...
| `/reviewit <pr>`       | Post-push AI review orchestrator. Lean default fires Gemini Flash + Copilot, handles Gemini first, then folds in Copilot, caps at 2 iterations. `deep` arg adds `/review` and restores 4-iter cap. Dedups, addresses each finding, replies in the PR thread. |
